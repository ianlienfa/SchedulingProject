# -*- coding: utf-8 -*-
"""
Created on Mon Jan 17 11:32:06 2022
...
def gene_algorithm(G,P,S,M):#generation繁衍几代，population人数，suviverate存活率,mutation突变率
    seq=np.arange(s_num)
    population=np.array([np.random.permutation(seq) for i in range(P)])
    fit=np.zeros(P)  
    for g in range(G):
        fit=[outtree_schedule(population[p].tolist()).cal_C() for p in range(P)]#每个人的sigC算出来，当成fitness
        sort_index=np.argsort(fit[:])#按sigC从小到大排
        if (g+1)%5==0:#每过几个可以输出一下看一下结果
            print(fit[sort_index[0]])
        population=population[sort_index,:]
        survive,mutation=round(P*S),round(P*M)
        for i in range(survive,P):#交配，sigC大的的覆蓋掉
            fatherindex=motherindex=np.random.randint(survive)#随机一个爸妈
            while(fatherindex==motherindex):
                motherindex=np.random.randint(survive)
            ...
            population[i,:]=[remain.pop() if son[k]==-1 else son[k] for k in range(s_num)]#没用到的塞进儿子,且population[i,:]=son=[.....]
    ...
    fit=[outtree_schedule(population[p].tolist()).cal_C() for p in range(P)]       
    sort_index=np.argsort(fit[:])#按sigC从小到大排
    population=population[sort_index,:]
    return population[0].tolist()
for i in range(1):
    l=gene_algorithm(100,500,0.3,0.002)
    # print(l)
    print(outtree_schedule(l).cal_C())
# f.close() #关闭文件
#14784. [27, 26, 19, 4, 11, 20, 14, 15, 2, 28, 10, 29, 22, 16, 23, 25, 17, 13, 6, 0, 5, 1, 12, 8, 21, 3, 18, 7, 24, 9]